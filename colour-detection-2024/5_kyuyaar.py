...
def draw_arrow_down(image, vertices, colourPoint):
    if len(vertices) == 1 and len(vertices[0]) >= 4:
        ...
        detection_points_arr = [
            [detect_x-10, detect_y+10], # top_left
            [detect_x+10, detect_y+10], # top_right
            [detect_x+10, detect_y-10], # bottom_left            
            [detect_x-10, detect_y-10], # bottom_right
            ]
...
def get_color_in_area(image_path, points):
    # Load the image
    image = cv2.imread(image_path)
...
points = draw_arrow_down(qr_image_with_boundary, qr_boundary_vertices, 1.2)

colour = get_color_in_area(image_path, points)
print(colour)